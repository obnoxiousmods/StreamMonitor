    ...
    if "streams_total" not in result:
        try:
            proc = await asyncio.create_subprocess_exec(
                ...
                "-U", "mediafusion", "-d", "mediafusion", "-t", "-A", "-c",
                "SELECT "
...
async def collect_zilean() -> dict:
    ...
    if isinstance(sample, list):
        ...
        dates = [e.get("ingested_at") or e.get("createdAt") for e in sample if e.get("ingested_at") or e.get("createdAt")]
        if dates:
    ...
    try:
        proc = await asyncio.create_subprocess_exec(
            "psql", "-h", "127.0.0.1", "-p", "6432",
            "-U", "zilean", "-d", "zilean", "-t", "-A", "-c",
            "SELECT \"Resolution\", COUNT(*) FROM \"Torrents\" WHERE \"Resolution\" IN ('1080p','2160p','720p','480p','unknown') GROUP BY \"Resolution\" ORDER BY COUNT(*) DESC;",
            stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE,
        )
        out, _ = await asyncio.wait_for(proc.communicate(), timeout=5)
        quality_dist = {}
...
async def _arr(base: str, key: str, *, path: str = "v3") -> dict:
    ...
    if isinstance(queue, dict):
        ...
        result["queue_errors"] = sum(1 for r in recs if r.get("status") in ("warning","error") or r.get("errorMessage"))
    if isinstance(health, list):
        result["health_errors"]   = sum(1 for h in health if h.get("type") == "error")
        result["health_warnings"] = sum(1 for h in health if h.get("type") == "warning")
        result["health_messages"] = [h.get("message","") for h in health if h.get("type")=="error"][:3]
...
async def collect_plex() -> dict:
    ...
    if isinstance(sec_txt, str):
        try:
            ...
            libs = [{"title": d.get("title",""), "type": d.get("type",""),
...
async def collect_qbittorrent() -> dict:
    ...
    async with httpx.AsyncClient(timeout=8) as c:
        ...
        try:
            tor_r = await c.get(f"{base}/api/v2/torrents/info?filter=active",
                                headers=cookie_h, timeout=8)
            if tor_r.status_code == 200:
                active = tor_r.json()
                result["active_torrents"] = len(active)
                result["downloading"] = sum(
                    1 for t in active
                    if t.get("state", "").startswith("downloading") or t.get("state") == "stalledDL"
                )
                result["seeding"] = sum(
                    1 for t in active
                    if "upload" in t.get("state", "").lower() or t.get("state") == "stalledUP"
                )
        except Exception:
            pass
    return result
