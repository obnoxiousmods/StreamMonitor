...
def _stat_entry(label: str, path: str, exp_user: str, exp_group: str,
                exp_mode: int, section: str) -> dict:
    exp_mode_str = oct(exp_mode)[2:]
    p = Path(path)
    base = {
        "label": label, "path": path, "section": section,
        "exp_user": exp_user, "exp_group": exp_group, "exp_mode": exp_mode_str,
    }
    if not p.exists():
        return {**base, "exists": False, "missing": True,
                "cur_user": "—", "cur_group": "—", "cur_mode": "—", "ok": False}
    try:
        s = p.stat()
        try:    cur_user  = pwd.getpwuid(s.st_uid).pw_name
        except KeyError: cur_user  = str(s.st_uid)
        try:    cur_group = grp.getgrgid(s.st_gid).gr_name
        except KeyError: cur_group = str(s.st_gid)
        cur_mode = oct(stat.S_IMODE(s.st_mode))[2:]
        ok = (cur_user == exp_user and cur_group == exp_group and cur_mode == exp_mode_str)
        return {**base, "exists": True, "missing": False,
                "cur_user": cur_user, "cur_group": cur_group, "cur_mode": cur_mode, "ok": ok}
...
def scan_perms() -> list[dict]:
    return [_stat_entry(lbl, path, u, g, m, sec)
            for lbl, path, u, g, m, sec in SCAN_TARGETS]
...
async def apply_fix(path: str, user: str, group: str, mode: str, recursive: bool) -> dict:
    ...
    try:
        proc = await asyncio.create_subprocess_exec(
            "sudo", "chown", *flags, f"{user}:{group}", path,
...
__all__ = ["scan_perms", "apply_fix", "SCAN_TARGETS"]
